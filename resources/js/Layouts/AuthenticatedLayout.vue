...
<template>
    ...
    <div v-if="$page.props.impersonation?.isImpersonating" class="bg-yellow-100 border-l-4 border-yellow-500 p-4">
            ...
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-sm text-yellow-700">
                        <strong>Impersonation Actief:</strong> Je bent ingelogd als <strong>{{ $page.props.auth.user.name }}</strong> 
                        namens <strong>{{ $page.props.impersonation.impersonator?.name }}</strong>
                    </p>
                </div>
            </div>
            <div class="ml-4">
                <form method="POST" action="/admin/stop-impersonation" class="inline">
                    <input type="hidden" name="_token" :value="$page.props.csrf_token">
                    <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm font-medium transition-colors">
                        Stop Impersonation
                    </button>
                </form>
            </div>
