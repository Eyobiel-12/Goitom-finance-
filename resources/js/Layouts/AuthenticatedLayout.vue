...
<template>
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:from-neutral-950 dark:via-neutral-950 dark:to-neutral-900">
                        ...
                        <div class="space-y-1">
                            <Link v-for="item in section.items" :key="item.name" :href="route(item.href)" @click="sidebarOpen = false" :aria-current="item.current() ? 'page' : undefined" :class="[
                                    item.current() 
                                        ...
                                        : 'border-transparent text-ink-700 hover:bg-gray-100 hover:text-ink-900 hover:shadow-sm dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-white',
                                    'group flex items-center px-4 py-3 text-base font-medium rounded-xl border-l-4 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-900'
                                ]">
                                <svg class="mr-3 h-5 w-5 flex-shrink-0 transition-transform duration-150 group-hover:scale-105" :class="item.current() ? 'text-white' : 'text-ink-500 dark:text-neutral-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div :class="[
            ...
            <div class="flex flex-grow flex-col overflow-y-auto bg-white/80 backdrop-blur border-r border-gray-200 dark:bg-neutral-900/80 dark:border-neutral-800 dark:text-neutral-200">
                <!-- Logo -->
                        ...
                        <div class="space-y-1">
                            <Link v-for="item in section.items" :key="item.name" :href="route(item.href)" :aria-current="item.current() ? 'page' : undefined" :class="[
                                    item.current() 
                                        ? 'relative bg-brand-600 text-white border-brand-600 shadow-cardStrong ring-1 ring-black/5 dark:ring-white/5' 
                                        : 'border-transparent text-ink-700 hover:bg-gray-100 hover:text-ink-900 hover:shadow-sm dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-white',
                                    'group flex items-center px-4 py-3 text-sm font-medium rounded-xl border-l-4 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-900',
                                    sidebarCollapsed ? 'justify-center px-3' : ''
                                ]" :title="sidebarCollapsed ? item.name : ''">
                                <svg class="h-5 w-5 flex-shrink-0 transition-transform duration-150 group-hover:scale-105" :class="item.current() ? 'text-white' : 'text-ink-500 dark:text-neutral-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="item.icon" />
                                </svg>
                ...
                <div class="flex shrink-0 border-t border-gray-200 p-4 dark:border-neutral-800">
                    <div v-if="!sidebarCollapsed" class="group block w-full flex-shrink-0">
                        ...
                        <div class="mt-2 flex space-x-2">
                            <Link :href="route('profile.edit')" class="text-xs text-ink-500 hover:text-ink-700 dark:text-neutral-400 dark:hover:text-neutral-200 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-900">Profiel</Link>
                            <Link :href="route('logout')" method="post" as="button" class="text-xs text-ink-500 hover:text-ink-700 dark:text-neutral-400 dark:hover:text-neutral-200 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-900">Uitloggen</Link>
                        </div>
                    </div>
